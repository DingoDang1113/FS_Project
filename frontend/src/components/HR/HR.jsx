...
const EmployeeList = ({ employees, displayTerminationDate }) => (
    <div className={displayTerminationDate ? 'Offboard-employees' : 'New-hires'}>
        ...
        <div className="field-name">{displayTerminationDate ? "Termination Date" : "Start Date"}</div>
        ...
        {employees.map(employee => (
            <div className='employee-row'>
                ...
                <input type="text" value={employee.positionDescription || employee.level} />
                <input type="date" value={displayTerminationDate ? employee.terminationDate : employee.startDate} />
            </div>
        ))}
    </div>
);
    ...
    useEffect(() => {
        fetchAllUsers()
            .then(users => {
                ...
                setNewHires(allUsers.filter(employee => employee.managerId === null && employee.employeeStatus === 'Active'));
                setLeavers(allUsers.filter(employee => employee.terminationDate !== null && employee.jobCode !== "L0000"));
            })
            .catch(error => {
